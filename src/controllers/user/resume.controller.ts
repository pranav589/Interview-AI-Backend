...
const normalizeBulletsForExport = (
  ...
  if (typeof updatedResumeSections === "object") {
    return Object.entries(updatedResumeSections as Record<string, string>).map(([original, improved]) => ({
...
export const analyzeResume = asyncHandler(async (req: Request, res: Response) => {
  ...
  */
  ...
  const job = await resumeJobService.createJob(userId, "resume-analysis", { resumeId });

  // Run analysis in background
  resumeAnalyzerService.analyze(userId, resumeId, resume.resumeText)
    .then(async (analysis) => {
      await resumeJobService.updateStatus(job._id.toString(), userId, "completed", {
    ...
    .catch(async (err) => {
      await resumeJobService.updateStatus(job._id.toString(), userId, "failed", {
  ...
  return res.status(202).json({
    success: true,
    message: "Analysis started in background. We'll notify you when it's ready.",
    data: {
      jobId: job._id,
      status: "processing",
    },
...
export const runJdMatch = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.file) {
    jobDescription = await jdMatchService.parseJdFile(req.file.path, req.file.mimetype);
  }

  if (!jobDescription) throw new ValidationError("Job description (text or file) is required");
  ...
  */
  ...
  const job = await resumeJobService.createJob(userId, "jd-match", { resumeId });

  // Run JD match in background
  jdMatchService.match(
    userId, 
    resumeId, 
    resume.resumeText, 
    jobDescription, 
  ...
  return res.status(202).json({
    success: true,
    message: "JD Matching started in background. We'll notify you when it's ready.",
    data: {
      jobId: job._id,
      status: "processing",
    },
...
export const startBuilder = asyncHandler(async (req: Request, res: Response) => {
...
export const sendBuilderMessage = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
  const { sessionId, message } = req.body;

  const session = await resumeBuilderService.processMessage(sessionId, userId, message);
...
export const exportResume = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!resumeData) throw new ValidationError("Resume data is required for export");
  const exportDate = new Date().toISOString().slice(0, 10);
  const builderFileName = `Resume_Builder_${exportDate}.pdf`;

  const pdfBuffer = await pdfExportService.generateResumePdf(resumeData, templateId || "modern");
  
  res.setHeader("Content-Type", "application/pdf");
  res.setHeader("Content-Disposition", `attachment; filename=${builderFileName}`);
  return res.send(pdfBuffer);
});

export const exportJdMatch = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
  const { id } = req.params;
  ...
  const resume = await resumeService.getResumeById(match.resumeId.toString(), userId);
  const exportDate = new Date().toISOString().slice(0, 10);
  ...
  if (match.shouldUpdateEntireResume) {
    sectionedSummary.push("Full Rewritten Resume", toSafeString(match.updatedResumeSections));
  } else {
    ...
    sectionedSummary.push("", "Original Resume Reference", toSafeString(match.resumeText));
  }

  resumeData.summary = sectionedSummary.filter(Boolean).join("\n");
  resumeData.skills = [...(match.matchedKeywords || []), ...(match.missingKeywords || []).map((kw: string) => `${kw} (gap)`)];
...
export const exportResumeAnalysis = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
  const { id } = req.params;

  const analysis = await resumeAnalyzerService.getAnalysisById(id, userId);
  const resume = await resumeService.getResumeById(analysis.resumeId.toString(), userId);
  const exportDate = new Date().toISOString().slice(0, 10);
  ...
  const reportData: AnalysisReportDocument = {
    ...
    overallAssessment: analysis.atsScore >= 80 
  ...
  const pdfBuffer = await pdfExportService.generateAnalysisReportPdf(reportData);
...
export const getUserAnalyses = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
...
export const getUserJdMatches = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
...
export const getResumeJob = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
  const { id } = req.params;
...
export const downloadResumeJobArtifact = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthenticatedRequest).user.id;
  const { id } = req.params;
  const job = await resumeJobService.getJobById(id, userId);
